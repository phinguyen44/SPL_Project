...
# NOTE: Please change the WD in order to run the file.
...
Probit models allow for the estimation of binary, random response variables [@schild2017]. Probit and Logit models mitigate problems that would occur in case a (linear) regression would be used for a similar task. In this case, the range of the response variable would not be restricted to the interval (0,1). Consequently, as probabilities are commonly defined on this space, non-interpretable probability estimations would be delivered. While literature often refers to use maximum likelihood to estimate coefficients, the `glm` function in `R` uses iterative re-weighted least squares [@stats]. The estimated coefficients are subject to a Wald Test on joint significance.

@schild2017 defines the model formula as
$$
P(Y = 1 | X = x) = \Phi (x' \beta) \tag{1}
$$
The error term is assumed to be independent and identically distributed (@wooldridge2012)
...
The marginal effect of $x_j$ on $P(Y = 1 | X = x)$ is defined as the effect of a ceteris paribus change of $x_j$ on the conditional probability that the response variable Y is realized as one. In case of a probit model, the marginal effect of $x_j$ on $P(Y = 1 | X = x)$ is dependent on x.
...
$$
PEA = \Phi(\hat{\beta_0} + \hat{\beta_1} \Sigma_{i = 1}^n x_i)\beta_1 \tag{2}
$$
...
$$
APE = \frac{1}{n} \Sigma_{i = 1}^n \Phi(\hat{\beta_0} + x_i \hat{\beta_1}) \beta_1 \tag{3}
$$
...
By means of the presented model, we evaluate how labor force participation behavior of individuals aged 50-64 is influenced by age and demographic as well as health indicators  across 11 European countries. @kalwij2005labour summarize their own results as follows:
...
xtab = xtable(mfx.Models[[1]]$fit, caption = "Marginal effects of health and age related variables on the probability of labor market participation of Austrian women")

print.xtable(xtab, caption.placement = "top", comment = FALSE)
```
...
This means `joint.wald.test` will conduct a joint significance test on all model coefficients by default. The Wald test statistic is calculated via simple matrix algebra based on the provided input by the model summary. This formula is equivalent to equation 6.
...
The general Wald test is designed in a similar manner, except it allows for the general formulation of linear hypothesis of the form $R \hat{\beta} = r$. The test statistic is given by equation 5. As in the `joint.wald.test` function, we use local local if-else statements to setup a default settings. If only the model summary is given as an input, the `general.wald.test` conducts a joint significance test at a 95% confidence level.
...
$$ \Pi_{icf} = Pr(Yi=1|Xi=xi_{cf}) = \Phi(xi_{cf}'{\beta}) \tag{7} $$
