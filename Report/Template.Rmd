...
@wooldridge2012 specifies two possible interpretations of the marginal effect. The "Partial Effect at the Average" (PEA) and  "Average Partial Effect". Following the approach of @kalwij2005labour, we focus on PEA, which denotes the marginal effect of a regressor on an average person ($\frac{1}{n}\Sigma_{x_i}$).
@dergutedietmar defines PEA it as

$$
PEA = \Phi(\hat{\beta_0} + \hat{\beta_1} \Sigma_{i = 1}^n x_i)\beta_1
$$
where $\beta$ is the vector of coefficients and $x_i$ is the vector of observations at index i. $\Phi$ represents the cumulative standard normal distribution function. Problematic are discrete $x_i$ because the resulting averages do not represent anybody in particular. Therefore, the marginal effects for binary variables are usually evaluated for the change from zero to one.
...
In the context of probit regression, the Wald test can be used to test multiple hypotheses regarding the model specifications. @kalwij2005labour conduct a Wald test to check the null hypothesis that none of the included health variables has an impact on labor participation in order to investigate the joint impact of health on participation.
...
$$    W = (R \hat{\beta} - r)'[R \hat{V}R']^{-1}(R \hat{\beta} - r) \tag{4} $$
...
$$   W = \hat{\beta}_s'[R\hat{V}R']^{-1}\hat{\beta}_s = \hat{\beta}_s'[\hat{V}_s]^{-1}\hat{\beta}_s \tag{5} $$

\subsection{Implementation}

To test the linear hypothesis regarding the model specifications, we design two Wald test versions. The function `joint.wald.test` tests the joint significance of a subset of model coefficients, whereas `general.wald.test` allows us to check any linear hypothesis. The two tests are designed in a way that they align well with the `glm` estimation output. The only required input is the model summary from `glm` estimation; the significance level and test specifications are optional.
...
knitr::kable(wald.men, caption = "Wald test on no joint impact of health on participation for men", digits = 2, align = "l")
```

Our results are in line with the findings of @kalwij2005labour. As expected, the null hypothesis of no impact of health on labor participation is rejected at a 95% confidence level for most countries. One difference is the inability to reject the null for France. The inability to reject the null for some countries is due to relatively low health-related probit coefficients (in absolute terms), which implies that these health indicators are not strongly related with the labor participation decision. 
...
$$ \Pi_{icf} = Pr(Yi=1|Xi=xi_{cf}) = \Phi(xi_{cf}'{\beta}) \tag{6} $$
...
knitr::kable(employment.age[12:22,], caption = "Men's current and counterfactual labor share for different age groups", digits = 2, align = "l")
```
