...
We use various variables on objective and subjective health related variables. The effects of older people generally having worse health conditions than younger one remain uncompensated. Sample size and variance differ among countries. Data may be affected by different national pension schemes, that incentivize workers to extend or restrict working depending on age. 


## Implementation

An efficient way of seperating the SHARE dataset into country, gender and age is explained in the following. In order to minimize hard-coding and possibly related errors, a grid is expanded between named variables.
...
The results can be easily bound into a data frame and arranged using `dplyr`. The same package is used to subset by gender. Using the pipe operator `%>%` keeps our code compact and avoids unnecessary variable assignment and inefficient memory requirements.

`vars` is actually a name vector that specifies whether labor force participation is full time, part time or not prevalent. Fetching it using `grep` from the cleaned data set makes it robust against name changes and ensures consistency. Conveniently, `grep` does not necessarily require regular expressions as an input. Instead, it is able to perform matches on simple strings and, in this case, returns the names that contain 'labor'. 
...
Having obtained the share of labor force participation grouped by gender, country and age as well as the according health variables, striking values can be presented.

Among the selected countries, France, Greece and Belgium have the fewest people among the oldest age group, which is ranging from 60 to 64 years. In the middle age group, ranging from 54-59, the German population is sparsely represented. Within the youngest age group, including people from 50 to 54 years old, Italy and Austria have the lowest share.  
Instead, Austria represents by far the largest share in the oldest age group. Belgium, Italy, the Netherlands and Sweden dominate the highest ranks in the group ranging from 54 to 59 years. Additionally, Greece solely leads the share in the youngest age group. 
...
Summary Statistics show that those countries, which strive in health topics are related to high labor force participation rates in all categories. Despite this, the only candidate for multicollinearity is the age dummy in the age group 50, which is omitted before model calculation. 
...
The underlying data enables us to mitigate justification bias and measurement error that occur when assessing health subjectively [@kalwij2005labour]. 

@kalwij2005labour summarize the results as follows:   
