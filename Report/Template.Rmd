...
@wooldridge2012 specifies two possible interpretations of the marginal effect. The "Partial Effect at the Average" (PEA) denotes the marginal effect of a regressor on an average person ($\frac{1}{n}\Sigma_{x_i}$). Problematic are discrete $x_i$ because the resulting averages do not represent anybody in particular. Furthermore, in case of a continuous explanatory variable wrapped inside a non-linear function, it remains unclear whether to apply the average over the function or insert the average into named function. We therefore focus on the "Average Partial/Marginal Effect" (APE), which describes the average effect of a regressor on the probability that $Y = 1$ of all individuals. @dergutedietmar defines it as

$$ APE = \frac{1}{n} \Sigma_{i = 1}^n \Phi(\hat{\beta_0} + x_i \hat{\beta_1}) \beta_1 \tag{2} $$
