...
An efficient way of separating the easySHARE dataset into country, gender and age is explained in the following. In order to minimize hard-coding and possibly related errors, a grid is expanded between named variables.
`labor`, a function that calculates labor participation rates can subsequently be applied onto the defined grid. 
...
As depicted in the table below, the mean value of chronic diseases is 75%. Conversely, 79% out of the 12689 participants have a good self-perceived health. At first sight, one may deduce that the majority considers themselves to be healthy despite the prevalence of a chronic disease. But the use of the arithmetic mean is sensitive towards outliers and the amount of chronic diseases is in fact counted numerically. In combination, these values indicate an uneven distribution. 
...
$$
The error term is assumed to be independent and identically distributed (@wooldrige2012)
...
The "Partial Effect at the Average" (PEA) denotes the marginal effect of a regressor on an average person ($\frac{1}{n}\Sigma_{x_i}$). Problematic are discrete $x_i$ because the resulting averages do not represent anybody in particular. Furthermore, in case of a continuous explanatory variable wrapped inside a non-linear function, it remains unclear whether to apply the average over the function or insert the average into named function. 

According to @dergutedietmar, the Partial Effect at the Average is defined as

$$
PEA = \Phi(\hat{\beta_0} + \hat{\beta_1} \Sigma_{i = 1}^n x_i)\beta_1
$$

The "Average Partial Effect"( or "Average Marginal Effect", APE) describes the effect of a regressor on the probability that $Y = 1$. 
...
By means of the presented model, we evaluate how labor force participation behavior of individuals aged 50-64 is influenced by age and demographic as well as health indicators  across 11 European countries.
...
knitr::kable(mfx.Models[[1]]$fit, caption = "Marginal effects of health and age related variables on the probability of labor market participation of Austrian women")
