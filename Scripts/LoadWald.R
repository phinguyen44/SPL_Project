...
# Confidence.level is the desired confidence level (between 0 and 1). Default: 0.95
...
joint.wald.test = function(model.summary, confidence.level = 0.95, spec = NULL){
  ...
  if(is.null(spec)){
    ...
    joint.wald.test[4] = ifelse(pval <= 1- confidence.level, "Reject H0", "Cannot reject H0")
    joint.wald.test
}
...
general.wald.test = function(model.summary, confidence.level = 0.95, R = NULL, r = NULL){
    ...
    # Set up test output
    chi2                 = qchisq(confidence.level, df=length(r))
